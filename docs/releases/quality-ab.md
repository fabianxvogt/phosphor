...
The same badge now switches to the acid accent and carries a concise hover explanation while the correction is active, then clears both when HD is selected or the Julia fit returns to a non-warning state. This keeps the visual cue discoverable without adding another render path or changing the bounded quality budgets.

Focus layout follow-up — **2026-09-17**: Julia and Fractal Flight quality-fit stages now use an explicit 16:10 aspect ratio, follow the capped width on narrow screens, and respect a viewport maximum. The width cap therefore cannot leave the original tall stage height in place and stretch a 16:10 render surface non-uniformly. This fixes geometric distortion; it does not create additional source pixels.
