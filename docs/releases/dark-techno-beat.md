...
Performance follow-up — **2026-09-17 (set timing pass)**: the output dock now offers a bounded **Measure 14 scenes** action. It warms each family once, records three timed draws at the selected output profile, fills the existing set-level p95 coverage, and restores the active scene, pause state, camera, audio source and authored set. A live browser pass reached `14/14` coverage; the active-beat run currently reports Cathedrals of Error as the slowest family at roughly `25.5ms` p95, so the result remains a tuning/rehearsal signal rather than a certification.
