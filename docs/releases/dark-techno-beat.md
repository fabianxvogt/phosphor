...
The source dock now names the active 16th-note step, voices, and bounded beat response while the demo is running; it returns to `BEAT IDLE` when the source stops. A compact `NEXT` readout names the following Demo step and voices as a read-only handoff cue, resetting to `NEXT —` when music stops or an external source is selected. A compact 16-step pattern monitor shows every kick, clap, hat/open-hat, sub, and ghost-percussion hit, highlights the scheduled step, and stays explicitly inactive for external audio. A companion `14/14 VISUALS BEAT-LINKED` readout makes the complete scene-family coverage explicit. Global pause now holds the Demo scheduler on its current step so audio and visuals resume together.
