...
Performance follow-up — **2026-09-17 (Cathedrals of Error)**: the renderer now reuses normalized ray directions, marches through scalar scratch state without per-pixel result objects, and uses a sharper `70%` internal raster with a bounded `12`-step beat-load march (`10` on Low); idle renders retain the authored full-detail raster. In the local Chrome rehearsal, the Demo-active path measured `14.4ms` p95 against the nominal `16.7ms` frame target. This is local empirical evidence, not a named-device certification.
