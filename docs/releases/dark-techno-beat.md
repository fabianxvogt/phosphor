...
Focus transport follow-up — **2026-09-17**: Focus now exposes a compact **Start demo beat** action whenever the source dock is hidden and no audio source is active. It enters the existing local Demo flow, disappears once any source is active, and stays unavailable during recording or offline rendering; external source selection and the saved set remain unchanged.

Focus beat handoff — **2026-09-17**: the stage overlay now mirrors the shared beat line while Focus is active, naming the current 16th-note step and voices for Demo playback (or the bounded response percentage for external audio). The readout is a dedicated, non-announcing row so it remains legible over the artwork without adding screen-reader chatter; idle and source-stop states remain explicit.
...
Verification follow-up — **2026-09-17 (response snapshot)**: the live fourteen-cell beat scope and the saved wiring check now consume one canonical per-family response snapshot. The contract test saturates low, mid, and high analyser bands and confirms all fourteen families report an active, bounded response while retaining their routed band labels. This prevents a future UI/evidence drift from making `14/14` look complete while a meter or report reads a different envelope.

Deployment follow-up — **2026-09-17**: the latest source is pushed to the owner branch and each publication retry is rebuilt from its exact pushed HEAD, but the configured Sites binding currently returns `project_not_found` when saving a version. The existing public preview therefore remains unchanged; no duplicate Site was created. Retry publication after the owner binding is restored.

The synthetic audio source is now a small local dark-techno instrument rather than a single sine pulse. One 16-step bar runs four-on-the-floor kick, clap backbeats, off-beat closed hats, occasional open hats, four low-level metallic ghost percussion ticks, and a restrained sub line. Percussive hits, including open hats, raise a bounded beat envelope; connected files, microphone, and tab audio continue to use analyser-derived bands instead of being mixed with the demo pattern. The round Start audio control now starts this local Demo beat when no source is connected, while an already-connected file, microphone, or tab source is only resumed. When a native source ends or returns no usable track, the control names the local Demo as a fallback.

Every Phosphor family receives the beat envelope in two ways: its existing scene-specific audio mapping gets a bounded transient boost, and the shared output receives a subtle accent pulse. Julia Observatory now routes its high-band mapping to the declared `24–112` iteration-detail control, quantizing the temporary lift to whole iterations before restoring the authored value; recursive filaments therefore gain a brief, bounded sharpness response instead of a barely visible motion nudge. Fractal Flight participates through surface detail and the shared pulse, but the beat never steers the camera. The pattern follows the BPM control and remains local-only; no audio is uploaded or persisted.

Schema-range follow-up — **2026-09-17**: transient beat modulation now derives its offset from the mapped control's declared minimum, maximum, and step. Normalized controls keep their prior response, while wide or signed controls remain valid if a future mapping targets them. The Julia detail mapping is the first user-facing use of this contract; authored session values remain untouched by the `finally` restoration guard. This is an incremental expressive-quality improvement, not a claim of added fractal resolution outside the beat frame.
...
The Demo kick now layers a low sine body beneath its pitch-dropping punch and sends the summed bus through a conservative dynamics compressor when the browser supports it. This adds weight and keeps the kick/clap stack inside a predictable headroom envelope; unsupported browsers retain the uncompressed fallback. The transport now exposes a bounded `Kick weight` control (`60–150%`, default `115%`) that changes only the local Demo kick and travels with portable sets; the active source status repeats the selected percentage so the low-end setting stays visible during a groove. Open hats add a short, quiet band-pass resonance under the airy high-pass tail, giving the deliberate beat accent a tactile metallic body without lengthening the high-end wash.
...
The source dock now names the active 16th-note step, voices, and bounded beat response while the demo is running; it returns to `BEAT IDLE` when the source stops. Compact `BAR` and `NEXT` readouts name the current bar and following Demo step as read-only handoff cues, resetting to `BAR —` / `NEXT —` when music stops or an external source is selected. A compact 16-step pattern monitor shows every kick, clap, hat/open-hat, sub, and ghost-percussion hit, highlights the scheduled step, and stays explicitly inactive for external audio. A companion `14/14 VISUALS BEAT-LINKED` readout makes the complete scene-family coverage explicit. Global pause now holds the Demo scheduler on its current step so audio and visuals resume together. Beat pulse setters and onset telemetry coerce hostile or non-finite values to a bounded zero before updating the transport or counting a hit, keeping diagnostic inputs from producing malformed readouts or telemetry.
