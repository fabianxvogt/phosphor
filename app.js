...
function syncDemoKickWeightReadout() { const input = $('demoKickWeightInput'); const output = $('demoKickWeightOutput'); const value = clamp(demoKickWeight, .6, 1.5); demoKickWeight = value; if (input) input.value = String(value); if (output) { const text = `${Math.round(value * 100)}%`; output.textContent = text; output.setAttribute('aria-label', `Demo kick weight ${Math.round(value * 100)} percent`); } }
