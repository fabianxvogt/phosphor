...
function syncStartAudioControl() { const button = $('startAudioButton'); if (!button) return; const active = audioSourceKind() !== 'NO AUDIO'; button.setAttribute('aria-pressed', String(active)); button.setAttribute('aria-label', active ? 'Audio active' : 'Start dark techno demo beat'); button.title = active ? 'Audio context and a source are active' : 'Start the local dark techno demo beat'; }
