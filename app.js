...
function syncQualityABReadout() { const output = $('qualityABReadout'); if (!output) return; if (!lastQualityABProbe) { output.textContent = scene().id === 'julia' ? 'Quality A/B not captured · Full vs HD' : 'Quality A/B available on Julia'; output.setAttribute('aria-label', scene().id === 'julia' ? 'Julia quality A/B has not been captured; compare Full and HD from this device' : 'Quality A/B is available when Julia is selected'); return; } const full = lastQualityABProbe.full; const hd = lastQualityABProbe.hd; const fullRenderer = qualityABRendererText(full); const hdRenderer = qualityABRendererText(hd); const recommendation = qualityABRecommendation(lastQualityABProbe); output.textContent = `A/B ${full.renderer.path.toUpperCase()} ${fullRenderer} ${full.medianMs.toFixed(1)}ms · HD ${hd.renderer.path.toUpperCase()} ${hdRenderer} ${hd.medianMs.toFixed(1)}ms${recommendation}`; output.setAttribute('aria-label', `Julia quality A/B captured on ${lastQualityABProbe.deviceLabel || 'this device'}; Full ${qualityABRendererAria(full)}, ${full.renderer.path}, ${full.medianMs.toFixed(1)} millisecond median; HD ${qualityABRendererAria(hd)}, ${hd.renderer.path}, ${hd.medianMs.toFixed(1)} millisecond median${recommendation ? `; recommendation ${recommendation.slice(3).toLowerCase()}` : ''}`); }
