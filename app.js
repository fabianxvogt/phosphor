...
function syncAudioSourceControls() { const locked = Boolean(audio.recorder || audio.recordingStream); const active = { demoAudioButton: state.demoOn, micButton: Boolean(audio.micStream), tabAudioButton: Boolean(audio.tabStream) }; const describedBy = 'audioStatus beatReadout audioCoverageReadout audioHeadroomReadout audioSessionReadout audioBeatTelemetryReadout'; for (const id of ['demoAudioButton', 'micButton', 'tabAudioButton', 'audioFileInput', 'stopAudioButton']) { const element = $(id); if (!element) continue; element.disabled = locked; element.setAttribute('aria-disabled', String(locked)); element.setAttribute('aria-describedby', describedBy); if (id in active) element.setAttribute('aria-pressed', String(Boolean(active[id]))); if (locked) element.title = 'Stop recording before changing audio source'; else element.removeAttribute?.('title'); } const sensitivity = $('audioSensitivity'); if (sensitivity) sensitivity.setAttribute('aria-describedby', describedBy); syncReadinessStatus(); }
...
function qualityABEffectiveBackingScale(measurement) {
  ...
  return Math.max(displayWidth / backingWidth, displayHeight / backingHeight);
}
...
function syncQualityABReadout() { const output = $('qualityABReadout'); if (!output) return; const probe = lastQualityABProbe || importedRehearsalReport?.qualityAB; const loaded = !lastQualityABProbe && Boolean(probe); if (!probe) { output.textContent = scene().id === 'julia' ? 'Quality A/B not captured · Full vs HD' : 'Quality A/B available on Julia'; output.setAttribute('aria-label', scene().id === 'julia' ? 'Julia quality A/B has not been captured; compare Full and HD from this device' : 'Quality A/B is available when Julia is selected'); return; } const full = probe.full; const hd = probe.hd; const fullRenderer = qualityABRendererText(full); const hdRenderer = qualityABRendererText(hd); const recommendation = qualityABRecommendation(probe); const prefix = loaded ? 'Loaded A/B' : 'A/B'; const stale = loaded && importedRehearsalReportStale ? ' · STALE' : ''; output.textContent = `${prefix} ${full.renderer.path.toUpperCase()} ${fullRenderer} ${full.medianMs.toFixed(1)}ms · HD ${hd.renderer.path.toUpperCase()} ${hdRenderer} ${hd.medianMs.toFixed(1)}ms${recommendation}${stale}`; output.setAttribute('aria-label', `${loaded ? 'Loaded ' : ''}Julia quality A/B captured on ${probe.deviceLabel || 'this device'}; Full ${qualityABRendererAria(full)}, ${full.renderer.path}, ${full.medianMs.toFixed(1)} millisecond median; HD ${qualityABRendererAria(hd)}, ${hd.renderer.path}, ${hd.medianMs.toFixed(1)} millisecond median${recommendation ? `; recommendation ${recommendation.slice(3).toLowerCase()}` : ''}${stale ? '; loaded evidence is stale' : ''}`); }
