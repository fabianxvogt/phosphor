...
import { audioBandLevels, DARK_TECHNO_PATTERN, darkTechnoStep, drivenRegimeFieldStep, drivenRegimeTarget, aquariumFoodStep, boundedFeedbackValue, cathedralEdgeEnhance, cathedralShading, clamp, countPolylineIntersections, coupledRegimeFieldStep, evolutionContour, finiteArray, fractalRenderSize, filteredInterference, interferenceField, lifecycleStressCheck, PHOSPHOR_FAMILY_CATALOG, qualityProfile, rayMarchCorridor, reactionDiffusionStep, resolutionAwareInterferenceFilter, seededRandom, stepElementary, topologyClosureError, topologyLoopPoint } from './core.mjs';
...
let cathedralLumaCache = new Float32Array(0);
let cathedralLumaCacheWidth = 0;
let cathedralLumaCacheHeight = 0;
let cathedralMarchDistance = 0;
let cathedralMarchSteps = 0;
let cathedralMarchHit = false;
let cathedralMarchCoverage = 0;
...
function ensureCathedralLumaCache(width, height) {
  if (cathedralLumaCacheWidth === width && cathedralLumaCacheHeight === height) return;
  cathedralLumaCacheWidth = width;
  cathedralLumaCacheHeight = height;
  cathedralLumaCache = new Float32Array(width * height);
}
function marchCathedral(originX, originY, originZ, directionX, directionY, directionZ, family, recursion, steps) { let distance = 0; cathedralMarchCoverage = 0; const depthFrequency = 1.2 + family * .4; for (let step = 0; step < steps; step += 1) { const x = originX + directionX * distance; const y = originY + directionY * distance; const z = originZ + directionZ * distance; let px = x; let py = y; for (let fold = 0; fold < recursion; fold += 1) { const scale = 1.45 + fold * .18; px = Math.abs(((px + .5 * scale) % scale) - .5 * scale) - .16; py = Math.abs(((py + .5 * scale) % scale) - .5 * scale) - .16; const swap = family === 1 || (family === 3 && fold % 2 === 1); if (swap) { const nextPx = px; px = py; py = nextPx; } } const radial = Math.hypot(px, py); const corridor = family === 0 ? Math.max(Math.abs(px), Math.abs(py)) - .06 : family === 1 ? radial - .08 : family === 2 ? Math.abs(px) + Math.abs(py) - .095 : Math.max(Math.abs(px + py) * .7, Math.abs(px - py) * .7) - .07; const depth = Math.abs(Math.sin(z * depthFrequency)) * .018; const fieldDistance = Math.max(.002, corridor + depth); if (!Number.isFinite(fieldDistance)) { cathedralMarchHit = false; cathedralMarchCoverage = 0; cathedralMarchDistance = 6; cathedralMarchSteps = step + 1; return; } if (fieldDistance < .006) { cathedralMarchHit = true; cathedralMarchCoverage = 1; cathedralMarchDistance = distance; cathedralMarchSteps = step + 1; return; } if (fieldDistance < .022) { cathedralMarchHit = false; cathedralMarchCoverage = clamp((.022 - fieldDistance) / .016, 0, 1); cathedralMarchDistance = distance; cathedralMarchSteps = step + 1; return; } distance += clamp(fieldDistance * .72, .004, .22); if (distance > 6) { cathedralMarchHit = false; cathedralMarchCoverage = 0; cathedralMarchDistance = distance; cathedralMarchSteps = step + 1; return; } } cathedralMarchHit = false; cathedralMarchCoverage = 0; cathedralMarchDistance = distance; cathedralMarchSteps = steps; }
...
function syncFocusAudioAction() { const button = $('focusAudioButton'); if (!button) return; const active = audioSourceKind() !== 'NO AUDIO'; const blocked = Boolean(offlineFrameJob || audio.recorder || audio.recordingStream); const visible = Boolean(state.focusMode && !active && !blocked); button.hidden = !visible; button.disabled = blocked; const fallback = ['empty', 'ended', 'failed'].includes(audioSourceOutcome); const label = fallback ? 'Start the dark techno demo beat as a fallback' : 'Start the dark techno demo beat'; button.textContent = fallback ? 'Start fallback beat' : 'Start demo beat'; button.setAttribute('aria-label', label); button.title = label; }
function syncAudioSourceControls() { const locked = Boolean(audio.recorder || audio.recordingStream); const active = { demoAudioButton: state.demoOn, micButton: Boolean(audio.micStream), tabAudioButton: Boolean(audio.tabStream) }; const describedBy = 'audioStatus beatReadout beatBarReadout beatNextReadout audioCoverageReadout audioHeadroomReadout audioSessionReadout audioBeatTelemetryReadout audioSourceHistoryReadout demoKickWeightOutput'; for (const id of ['demoAudioButton', 'micButton', 'tabAudioButton', 'audioFileInput', 'stopAudioButton']) { const element = $(id); if (!element) continue; element.disabled = locked; element.setAttribute('aria-disabled', String(locked)); element.setAttribute('aria-describedby', describedBy); if (id in active) element.setAttribute('aria-pressed', String(Boolean(active[id]))); if (locked) element.title = 'Stop recording before changing audio source'; else element.removeAttribute?.('title'); } const sensitivity = $('audioSensitivity'); if (sensitivity) sensitivity.setAttribute('aria-describedby', describedBy); const kick = $('demoKickWeightInput'); if (kick) { kick.disabled = locked; kick.setAttribute('aria-disabled', String(locked)); kick.setAttribute('aria-describedby', describedBy); } syncDemoKickWeightReadout(); syncStartAudioControl(); syncFocusAudioAction(); syncReadinessStatus(); }
function demoAudioStatusLabel() { return `Dark techno demo beat · kick ${Math.round(demoKickWeight * 100)}% · clap, hats, perc · tempo follows BPM`; }
function syncDemoKickWeightReadout() { const input = $('demoKickWeightInput'); const output = $('demoKickWeightOutput'); const value = clamp(demoKickWeight, .6, 1.5); demoKickWeight = value; if (input) input.value = String(value); if (output) { const text = `${Math.round(value * 100)}%`; output.textContent = text; output.setAttribute('aria-label', `Demo kick weight ${Math.round(value * 100)} percent`); } if (state.demoOn && $('audioStatus')) $('audioStatus').textContent = demoAudioStatusLabel(); }
...
function rendererReadoutText() {
  ...
  if (scene().kind === 'geometry' && (state.demoOn || state.audioBandsReady)) {
    ...
    const beatPlan = cathedralRenderPlan(profile, true);
    const beatWidth = beatPlan.width;
    const beatHeight = beatPlan.height;
...
function cathedralFocusRasterScale(profile = outputProfile()) { return state.focusMode && profile?.workScale === 1 ? 1.5 : 1; }
function cathedralRenderPlan(profile = outputProfile(), beatLoad = state.demoOn || state.audioBandsReady) {
  const profileScale = 1;
  const beatScale = beatLoad ? cathedralBeatRasterScale(profile) : 1;
  const focusScale = beatLoad ? 1 : cathedralFocusRasterScale(profile);
  const width = Math.min(profile.width, Math.max(1, Math.round(profile.cathedralWidth * profileScale * beatScale * focusScale)));
  const height = Math.min(profile.height, Math.max(1, Math.round(profile.cathedralHeight * profileScale * beatScale * focusScale)));
  return { width, height, scale: Number((width / Math.max(1, profile.cathedralWidth)).toFixed(3)), sourceWidth: profile.cathedralWidth, sourceHeight: profile.cathedralHeight, beatLoad: Boolean(beatLoad) };
}
function cathedralMaxSteps(profile = outputProfile(), beatLoad = state.demoOn || state.audioBandsReady) {
  if (beatLoad) return profile?.workScale < 1 ? 8 : 10;
  if (state.focusMode && profile?.workScale === 1) return 36;
  return profile?.workScale < 1 ? 32 : 48;
}
...
function syncFocusScaleReadout() {
  ...
  if (signature === lastDisplayScaleSignature) {
    ...
    syncQualityBadgeHint(showHdHint);
    return;
  }
...
const sanitizeRehearsalAudioCore = sanitizeRehearsalAudio;
sanitizeRehearsalAudio = function sanitizeRehearsalAudioWithKickWeight(data) { const audioData = sanitizeRehearsalAudioCore(data); if (!audioData) return audioData; const rawKickWeight = data?.demoKickWeight; if (rawKickWeight === undefined || rawKickWeight === null) return { ...audioData, demoKickWeight: null }; const safeKickWeight = normalizedBeatValue(rawKickWeight); if (safeKickWeight < .6 || safeKickWeight > 1.5) throw new Error('Rehearsal report Demo kick weight is malformed'); return { ...audioData, demoKickWeight: safeKickWeight }; };
...
function compareRehearsalReports(current, candidate) {
  ...
  const currentKickWeight = safeCurrent.audio?.demoKickWeight; const candidateKickWeight = safeCandidate.audio?.demoKickWeight; const sameAudioKickWeight = currentKickWeight === undefined || currentKickWeight === null || candidateKickWeight === undefined || candidateKickWeight === null || currentKickWeight === candidateKickWeight;
  return { sameSet, sameScene, sameProfile, sameTransport, sameDevice, sameEnvironment, sameRenderer, sameQualityAB, sameBeatResponse, samePreflight, sameRecording, sameObservations, sameAudio, sameAudioHistory, sameAudioPeak, sameAudioPeakSession, sameAudioBeat, samePerformance, samePerformanceSet, samePassSnapshot, sameAudioKickWeight, equivalent: sameSet && sameScene && sameProfile && sameTransport && sameDevice && sameEnvironment && sameRenderer && sameQualityAB && sameBeatResponse && samePreflight && sameRecording && sameObservations && sameAudio && sameAudioHistory && sameAudioPeak && sameAudioPeakSession && sameAudioBeat && samePerformance && samePerformanceSet && samePassSnapshot };
}
const compareRehearsalReportsCore = compareRehearsalReports;
compareRehearsalReports = function compareRehearsalReportsWithKickWeight(current, candidate) { const comparison = compareRehearsalReportsCore(current, candidate); const sameAudioSource = comparison.sameAudio; const sameKickWeight = comparison.sameAudioKickWeight !== false; comparison.sameAudioSource = sameAudioSource; comparison.sameAudio = sameAudioSource && sameKickWeight; comparison.equivalent = comparison.equivalent && sameKickWeight; comparison.sameAudioKickWeight = sameKickWeight; return comparison; };
...
function acidRenderPlan(profile = outputProfile()) {
  const scale = profile.workScale >= 2 ? 4 : profile.workScale >= 1 ? 2 : 1;
  return { width: Math.min(profile.width, buffers.acid.width * scale), height: Math.min(profile.height, buffers.acid.height * scale), scale, sourceWidth: buffers.acid.width, sourceHeight: buffers.acid.height };
}
function drawAcid() {
  const b = buffers.acid; const p = state.params.acid; const a = colorRgb(palette.primary); const s = colorRgb(palette.secondary); const c = colorRgb(palette.accent); const plan = acidRenderPlan(); const image = ctx.createImageData(plan.width, plan.height); const data = image.data; const sourceWidth = b.width; const sourceHeight = b.height; const xScale = (sourceWidth - 1) / Math.max(1, plan.width - 1); const yScale = (sourceHeight - 1) / Math.max(1, plan.height - 1);
  for (let y = 0; y < plan.height; y += 1) {
    const sourceY = y * yScale; const y0 = Math.floor(sourceY); const y1 = Math.min(sourceHeight - 1, y0 + 1); const fy = sourceY - y0;
    for (let x = 0; x < plan.width; x += 1) {
      const sourceX = x * xScale; const x0 = Math.floor(sourceX); const x1 = Math.min(sourceWidth - 1, x0 + 1); const fx = sourceX - x0; const top = b.v[y0 * sourceWidth + x0] * (1 - fx) + b.v[y0 * sourceWidth + x1] * fx; const bottom = b.v[y1 * sourceWidth + x0] * (1 - fx) + b.v[y1 * sourceWidth + x1] * fx; const field = top * (1 - fy) + bottom * fy;
      const leftTop = b.v[y0 * sourceWidth + Math.max(0, x0 - 1)]; const leftBottom = b.v[y1 * sourceWidth + Math.max(0, x0 - 1)]; const rightTop = b.v[y0 * sourceWidth + Math.min(sourceWidth - 1, x0 + 1)]; const rightBottom = b.v[y1 * sourceWidth + Math.min(sourceWidth - 1, x0 + 1)]; const upRow = Math.max(0, y0 - 1); const downRow = Math.min(sourceHeight - 1, y0 + 1); const up = b.v[upRow * sourceWidth + x0] * (1 - fx) + b.v[upRow * sourceWidth + x1] * fx; const down = b.v[downRow * sourceWidth + x0] * (1 - fx) + b.v[downRow * sourceWidth + x1] * fx; const left = leftTop * (1 - fy) + leftBottom * fy; const right = rightTop * (1 - fy) + rightBottom * fy; const edge = Math.abs(field - (left + right + up + down) * .25); const value = clamp((field + edge * (.18 + p.contrast * .08)) * p.contrast * 2.6, 0, 1); const m = value < .5 ? mixColor([5, 7, 12], s, value * 2) : mixColor(s, a, (value - .5) * 2); const tint = (Math.sin(x * .065 + y * .035 + state.elapsed * p.drift * 1.5) + 1) * .5; const rgb = mixColor(m, c, tint * value * .28); const index = (y * plan.width + x) * 4; data[index] = rgb[0]; data[index + 1] = rgb[1]; data[index + 2] = rgb[2]; data[index + 3] = 255;
    }
  }
  renderBuffer.width = plan.width; renderBuffer.height = plan.height; renderCtx.putImageData(image, 0, 0); ctx.imageSmoothingEnabled = true; ctx.drawImage(renderBuffer, 0, 0, canvas.width, canvas.height); renderBuffer.width = canvas.width; renderBuffer.height = canvas.height;
}
...
function phaseRenderPlan(profile = outputProfile()) {
  const scale = profile.workScale >= 2 ? 6 : profile.workScale >= 1 ? 4 : 2;
  return { width: Math.min(profile.width, buffers.phase.width * scale), height: Math.min(profile.height, buffers.phase.height * scale), scale, sourceWidth: buffers.phase.width, sourceHeight: buffers.phase.height };
}
function drawPhase() {
  const p = state.params.phase; const b = buffers.phase; const plan = phaseRenderPlan(); const image = ctx.createImageData(plan.width, plan.height); const data = image.data; const a = colorRgb(palette.secondary); const c = colorRgb(palette.accent); const primary = colorRgb(palette.primary); const xScale = (b.width - 1) / Math.max(1, plan.width - 1); const yScale = (b.height - 1) / Math.max(1, plan.height - 1);
  const sample = (field, x, y) => { const sourceX = clamp(x, 0, b.width - 1); const sourceY = clamp(y, 0, b.height - 1); const x0 = Math.floor(sourceX); const y0 = Math.floor(sourceY); const x1 = Math.min(b.width - 1, x0 + 1); const y1 = Math.min(b.height - 1, y0 + 1); const fx = sourceX - x0; const fy = sourceY - y0; const top = field[y0 * b.width + x0] * (1 - fx) + field[y0 * b.width + x1] * fx; const bottom = field[y1 * b.width + x0] * (1 - fx) + field[y1 * b.width + x1] * fx; return top * (1 - fy) + bottom * fy; };
  for (let y = 0; y < plan.height; y += 1) {
    const sourceY = y * yScale;
    for (let x = 0; x < plan.width; x += 1) {
      const sourceX = x * xScale; const field = p.compare > .5 && x >= plan.width / 2 ? b.compare : b.values; const center = sample(field, sourceX, sourceY); const left = sample(field, sourceX - 1, sourceY); const right = sample(field, sourceX + 1, sourceY); const up = sample(field, sourceX, sourceY - 1); const down = sample(field, sourceX, sourceY + 1); const edge = Math.abs(center - (left + right + up + down) * .25); const value = clamp(center + edge * (.08 + p.coupling * .08), 0, 1); const rgb = value < .5 ? mixColor(a, c, value * 2) : mixColor(c, primary, (value - .5) * 2); const index = (y * plan.width + x) * 4; data[index] = rgb[0]; data[index + 1] = rgb[1]; data[index + 2] = rgb[2]; data[index + 3] = 255;
    }
  }
  renderBuffer.width = plan.width; renderBuffer.height = plan.height; renderCtx.putImageData(image, 0, 0); ctx.imageSmoothingEnabled = true; ctx.drawImage(renderBuffer, 0, 0, canvas.width, canvas.height); if (p.compare > .5) { ctx.strokeStyle = palette.primary; ctx.globalAlpha = .6; ctx.beginPath(); ctx.moveTo(canvas.width / 2, 0); ctx.lineTo(canvas.width / 2, canvas.height); ctx.stroke(); ctx.globalAlpha = 1; }
}
...
function stepAndDraw(dt) {
  const def = scene(), id = def.id, original = state.params[id];
  ...
  if (mapping) {
    ...
    if (mappedLevel > 0) {
      ...
      original[key] = beatMappedParameterValue(def, key, mappedValue, mappedLevel, amount);
    }
  }
...
function normalizedBeatValue(value = 0) {
  let numeric = 0;
  try { numeric = Number(value); } catch {}
...
function setBeatPulse(amount, step = state.beatStep) { const safeAmount = clamp(normalizedBeatValue(amount), 0, 1); const rawStep = normalizedBeatIndex(step); const safeStep = ((rawStep % DARK_TECHNO_PATTERN.length) + DARK_TECHNO_PATTERN.length) % DARK_TECHNO_PATTERN.length; state.beatPulse = Math.max(state.beatPulse, safeAmount); state.beatStep = safeStep; state.beatBar = Math.floor(Math.max(0, rawStep) / DARK_TECHNO_PATTERN.length); if (state.demoOn) recordDemoBeatOnset(safeStep, safeAmount); syncBeatReadout(); }
...
function beatMappedParameterValue(def, key, value, level, amount) {
  const schema = def?.schema?.find(([schemaKey]) => schemaKey === key);
  const minimum = Number.isFinite(schema?.[2]) ? Number(schema[2]) : 0;
  const maximum = Number.isFinite(schema?.[3]) ? Number(schema[3]) : 1;
  const span = Math.max(0, maximum - minimum);
  const base = clamp(normalizedBeatValue(value), minimum, maximum);
  const pulse = clamp(normalizedBeatValue(level), 0, 1);
  const strength = clamp(normalizedBeatValue(amount), 0, 1);
  const raw = clamp(base + pulse * strength * span, minimum, maximum);
  const step = Number.isFinite(schema?.[4]) && Number(schema[4]) > 0 ? Number(schema[4]) : 0;
  if (!step) return raw;
  return clamp(minimum + Math.round((raw - minimum) / step) * step, minimum, maximum);
}
...
function playDemoKick(start, velocity, weight = demoKickWeight) {
  ...
  const safeWeight = clamp(normalizedBeatValue(weight), .6, 1.5);
  demoOscillator(start, .32, (.44 + velocity * .22) * safeWeight, 'sine', 112, 36);
  demoOscillator(start, .25, (.56 + velocity * .24) * safeWeight, 'sine', 148, 44);
  demoOscillator(start, .045, (.14 + velocity * .08) * safeWeight, 'square', 1800, 420);
}
...
function demoStepPulse(step) {
  let pulse = 0;
  if (step?.kick) pulse = Math.max(pulse, .86 + step.kick * .14);
  if (step?.clap) pulse = Math.max(pulse, .7);
  if (step?.hat) pulse = Math.max(pulse, .24 + step.hat * .18);
  if (step?.openHat) pulse = Math.max(pulse, .32 + step.openHat * .22);
  if (step?.perc) pulse = Math.max(pulse, .16 + step.perc * .12);
...
function scheduleDemoStep(request) {
  ...
  const pulse = demoStepPulse(step);
  if (pulse > 0) setBeatPulse(pulse, absoluteStep);
  if (step.kick) playDemoKick(start, step.kick, demoKickWeight);
  if (step.bass) playDemoBass(start, stepIndex, step.bass);
  if (step.clap) playDemoClap(start);
  if (step.hat) playDemoHat(start, step.hat);
  if (step.openHat) playDemoHat(start, step.openHat, true);
  if (step.perc) playDemoPerc(start, step.perc);
...
async function toggleDemo() {
  ...
  scheduleDemoStep(request); setAudioSourceOutcome('active', 'DEMO'); $('demoAudioButton').textContent = 'Stop beat'; $('audioStatus').textContent = demoAudioStatusLabel(); syncAudioSourceControls();
}
...
const validateSessionWithDeviceLabelCore = validateSession;
validateSession = function validateSessionWithKickWeight(data) { const candidate = validateSessionWithDeviceLabelCore(data); const rawKickWeight = candidate.options?.demoKickWeight; if (rawKickWeight !== undefined) { const safeKickWeight = normalizedBeatValue(rawKickWeight); if (safeKickWeight < .6 || safeKickWeight > 1.5) throw new Error('Session kick weight is malformed'); candidate.options.demoKickWeight = safeKickWeight; } return candidate; };
...
const rehearsalReportWithDeviceLabelCore = rehearsalReport;
rehearsalReport = function rehearsalReportWithKickWeight() { const report = rehearsalReportWithDeviceLabelCore(); report.audio = { ...report.audio, demoKickWeight }; return report; };
...
const applySessionWithDeviceLabelCore = applySession;
applySession = function applySessionWithKickWeight(data, internalRestore = false) { const previousKickWeight = demoKickWeight; const result = applySessionWithDeviceLabelCore(data, internalRestore); demoKickWeight = clamp(normalizedBeatValue(data?.options?.demoKickWeight ?? 1.15), .6, 1.5); syncDemoKickWeightReadout(); if (demoKickWeight !== previousKickWeight) { markRehearsalReportStale(); if (!internalRestore) saveLocal(); } return result; };
...
const setOfflineControlsLockedWithDeviceLabelCore = setOfflineControlsLocked;
setOfflineControlsLocked = function setOfflineControlsLockedWithKickWeight(job, locked) { const input = $('demoKickWeightInput'); if (input) { if (locked) job.controlState.set(input, input.disabled); input.disabled = locked ? true : job.controlState.get(input) ?? false; } return setOfflineControlsLockedWithDeviceLabelCore(job, locked); };
...
window.__phosphorTest.cathedralEdgeEnhance = cathedralEdgeEnhance;
...
boot();
const demoKickWeightInput = $('demoKickWeightInput');
demoKickWeightInput?.addEventListener('input', (event) => { if (offlineJobActive()) return; demoKickWeight = clamp(normalizedBeatValue(event.target.value), .6, 1.5); syncDemoKickWeightReadout(); markDirty(); });
const focusAudioButton = $('focusAudioButton');
focusAudioButton?.addEventListener('click', startAudioPlayback);
