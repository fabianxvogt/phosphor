...
function syncAudioSourceControls() { const locked = Boolean(audio.recorder || audio.recordingStream); const active = { demoAudioButton: state.demoOn, micButton: Boolean(audio.micStream), tabAudioButton: Boolean(audio.tabStream) }; const describedBy = 'audioStatus beatReadout audioCoverageReadout audioHeadroomReadout audioSessionReadout'; for (const id of ['demoAudioButton', 'micButton', 'tabAudioButton', 'audioFileInput', 'stopAudioButton']) { const element = $(id); if (!element) continue; element.disabled = locked; element.setAttribute('aria-disabled', String(locked)); element.setAttribute('aria-describedby', describedBy); if (id in active) element.setAttribute('aria-pressed', String(Boolean(active[id]))); if (locked) element.title = 'Stop recording before changing audio source'; else element.removeAttribute?.('title'); } const sensitivity = $('audioSensitivity'); if (sensitivity) sensitivity.setAttribute('aria-describedby', describedBy); syncReadinessStatus(); }
...
function syncQualityABReadout() { const output = $('qualityABReadout'); if (!output) return; if (!lastQualityABProbe) { output.textContent = scene().id === 'julia' ? 'Quality A/B not captured · Full vs HD' : 'Quality A/B available on Julia'; output.setAttribute('aria-label', scene().id === 'julia' ? 'Julia quality A/B has not been captured; compare Full and HD from this device' : 'Quality A/B is available when Julia is selected'); return; } const full = lastQualityABProbe.full; const hd = lastQualityABProbe.hd; output.textContent = `A/B ${full.renderer.path.toUpperCase()} ${full.width}×${full.height} ${full.medianMs.toFixed(1)}ms · HD ${hd.renderer.path.toUpperCase()} ${hd.width}×${hd.height} ${hd.medianMs.toFixed(1)}ms`; output.setAttribute('aria-label', `Julia quality A/B captured on ${lastQualityABProbe.deviceLabel || 'this device'}; Full ${full.width} by ${full.height}, ${full.renderer.path}, ${full.medianMs.toFixed(1)} millisecond median; HD ${hd.width} by ${hd.height}, ${hd.renderer.path}, ${hd.medianMs.toFixed(1)} millisecond median`); }
...
function syncAudioSourceOutcome() { const output = $('audioOutcomeReadout'); if (!output) return; const label = audioSourceOutcomeLabels[audioSourceOutcome] || audioSourceOutcomeLabels.idle; if (output.textContent !== label) output.textContent = label; output.setAttribute('aria-label', label); }
function setAudioSourceOutcome(next, source = audioSourceKind()) { audioSourceOutcome = audioSourceOutcomes.has(next) ? next : 'idle'; recordAudioSourceEvent(source, audioSourceOutcome); syncAudioSourceOutcome(); syncReadinessStatus(); }
...
function sanitizeRehearsalAudio(data) {
  ...
  return { source: data.source, status, history, peak, peakSession };
}
...
function compareRehearsalReports(current, candidate) {
  ...
  return { sameSet, sameScene, sameProfile, sameTransport, sameDevice, sameEnvironment, sameRenderer, sameQualityAB, sameBeatResponse, samePreflight, sameRecording, sameObservations, sameAudio, sameAudioHistory, sameAudioPeak, sameAudioPeakSession, samePerformance, samePerformanceSet, samePassSnapshot, equivalent: sameSet && sameScene && sameProfile && sameTransport && sameDevice && sameEnvironment && sameRenderer && sameQualityAB && sameBeatResponse && samePreflight && sameRecording && sameObservations && sameAudio && sameAudioHistory && sameAudioPeak && sameAudioPeakSession && samePerformance && samePerformanceSet && samePassSnapshot };
}
...
function syncRehearsalReportImportComparison(comparison = null) { const output = $('rehearsalReportImportCompareReadout'); if (!output) return; if (!comparison) { output.textContent = 'No loaded comparison'; output.setAttribute('aria-label', 'No loaded rehearsal report comparison'); delete output.dataset.level; return; } const groups = [['setup', comparison.sameDevice && comparison.sameEnvironment], ['transport', comparison.sameTransport], ['audio', comparison.sameAudio && comparison.sameAudioHistory && comparison.sameAudioPeak && comparison.sameAudioPeakSession && comparison.sameRecording], ['timing', comparison.samePerformance && comparison.samePerformanceSet], ['visuals', comparison.sameScene && comparison.sameProfile && comparison.sameRenderer && comparison.sameQualityAB && comparison.sameBeatResponse], ['evidence', comparison.samePreflight && comparison.sameObservations], ['pass', comparison.samePassSnapshot], ['set', comparison.sameSet]]; const differences = groups.filter(([, same]) => !same).map(([label]) => label); if (!differences.length) { output.textContent = 'Compare · live match'; output.setAttribute('aria-label', 'Loaded rehearsal report matches the live setup, transport, audio, timing, visuals, evidence, pass, and set'); output.dataset.level = 'match'; return; } output.textContent = `Compare · differs: ${differences.join(' · ')}`; output.setAttribute('aria-label', `Loaded rehearsal report differs from the live instrument in ${differences.join(', ')}`); output.dataset.level = 'differs'; }
...
function writeRehearsalReportCache() { try { localStorage.setItem(rehearsalReportCacheKey, JSON.stringify({ format: rehearsalReportCacheKey, version: 1, reportStamp: lastRehearsalReportAt, signature: lastRehearsalReportSignature, liveSignature: lastRehearsalReportLiveSignature, performanceSignature: lastRehearsalReportPerformanceSignature, performanceSetSignature: lastRehearsalReportPerformanceSetSignature, audioPeakSessionSignature: lastRehearsalReportAudioPeakSessionSignature, deviceLabel: lastRehearsalReportDeviceLabel, recordingMimeType: lastPreflightRecordingMime, recordingStatus: recordingOutcome, preflight: lastPreflightReport ? structuredClone(lastPreflightReport) : null, preflightCheckedAt: lastPreflightAt, observedChecks: { ...state.rehearsalChecks }, observedChecksAt: state.rehearsalChecksAt })); } catch {} }
...
function stopAudioSource(status = 'No music connected') {
  ...
  audio.micStream = null; audio.tabStream = null; state.demoOn = false; state.audioLevel = 0; state.audioPeak = 0; state.audioPeakHold = 0; resetAudioPeakSession(); state.beatPulse = 0; state.beatStep = 0; state.beatBar = 0;
...
async function ensureAudio(source = audioSourceKind()) {
  ...
  try {
    ...
    $('startAudioButton').textContent = '◉'; return true;
  } catch { setAudioSourceOutcome('failed', source); $('audioStatus').textContent = 'Audio unavailable · visual controls still work'; return false; }
}
...
async function loadLocalAudio(file) {
  ...
  media.onerror = fail;
  try { audio.source = audio.context.createMediaElementSource(media); audio.source.connect(audio.analyser); await media.play(); if (request !== audioRequest) { release(); return; } setAudioSourceOutcome('active', 'FILE'); $('audioStatus').textContent = `Playing ${file.name} · local file`; } catch { if (request !== audioRequest) release(); else fail(); }
}
...
window.__phosphorTest = { stepFlight, stopFlight, flightState: () => ({ pose: structuredClone(flightPose), cruise: flightCruise, blocked: flightBlocked, keys: [...flightKeys] }), sceneDefs, performanceScoreCues, PHOSPHOR_FAMILY_CATALOG, audioBandLevels, audioResponseLevel, darkTechnoStep, visualAudioCoverage, beatDrivenEffects, applyBeatVisualPulse, runBeatResponseCheck, sanitizeBeatResponseCheck, setCueLabel, setCueDuration, moveCue, previewCue, duplicateCue, clearRehearsalReportImport, stepElementary, reactionDiffusionStep, finiteArray, boundedFeedbackValue, lifecycleStressCheck, qualityProfile, cathedralShading, aquariumFoodStep, evolutionContour, runLifecycleProbe, runQualityABProbe, sanitizeQualityAB, sessionData, validateSession, applySession, switchScene, stepPhase, phaseMeasurement, startPhaseArc, stopPhaseArc, rehearsePhaseArcs, archivePhaseMeasurement, frameManifest, validateFrameManifest, importFrameManifest, frameDirectoryWriter, renderOfflineFrames, cancelOfflineRender, mutateEvolution, chooseEvolutionChild, promoteEvolution, inject, stepMagnetic, stopMagneticReplay, replayGestureSequence, magneticReplayState: () => ({ index: buffers.magnetic.replay.index, total: buffers.magnetic.replay.events.length, playing: buffers.magnetic.replay.playing }), evolutionRenderState: () => ({ phase: buffers.evolution.phase, siblings: Array.from(buffers.evolution.siblings) }), magneticRenderState: () => ({ phase: buffers.magnetic.phase, particles: Array.from(buffers.magnetic.particles), previous: Array.from(buffers.magnetic.previous), attractors: Array.from(buffers.magnetic.attractors) }), interferenceRenderState: () => ({ phase: buffers.interference.phase }), stagePixelStats, renderRuntimeState, restoreOfflineSnapshot, setTestRenderFlags, setTestElapsed: (value) => { state.elapsed = Math.max(0, Number(value) || 0); }, setTestAudioLevel: (value) => { state.audioLevel = Math.max(0, Number(value) || 0); }, setTestAudioPeak: (value, hold = value, record = false, elapsedSeconds = 0) => { state.audioPeak = clamp(Number(value) || 0, 0, 1); state.audioPeakHold = clamp(Number(hold) || 0, 0, 1); if (record) recordAudioPeakSample(state.audioPeak, state.audioPeakHold, elapsedSeconds); syncAudioHeadroomReadout(); }, recordAudioPeakSample, audioPeakSessionTelemetry, rehearsalPassSummary, setTestAudioBands: (value) => { state.audioBands = { low: clamp(Number(value?.low), 0, 1), mid: clamp(Number(value?.mid), 0, 1), high: clamp(Number(value?.high), 0, 1) }; state.audioBandsReady = true; }, setTestBeatPulse: (value, step = 0) => { state.beatPulse = clamp(Number(value) || 0, 0, 1); state.beatStep = ((Math.floor(Number(step) || 0) % 16) + 16) % 16; syncBeatReadout(); } };
Object.assign(window.__phosphorTest, { drawPreview, stepAcid, resetAcid, acidRenderState: () => ({ u: Array.from(buffers.acid.u), v: Array.from(buffers.acid.v) }), pendingFramePlan: () => pendingFrameManifest, toggleRecord, finishRecording, syncRecordingReadout, updatePerformanceReadout, syncSetPerformanceReadout, performanceSetSummary: () => metrics.setSummary(), syncFocusScaleReadout, syncFocusRenderFit, rehearsalReport, exportRehearsalReport, validateRehearsalReport, compareRehearsalReports, importRehearsalReport, readRehearsalReportFile, rehearsalReportImport: () => structuredClone(importedRehearsalReport), restoreRehearsalReportCache, resetObservedChecks, recordingState: () => ({ recorder: audio.recorder, chunks: audio.chunks.length, stream: audio.recordingStream, startedAt: audio.recordingStartedAt }), stepAndDraw, updateAudioLevel, stopAudioSource, toggleDemo, loadLocalAudio, toggleMic, connectTabAudio, preflightReport, runPreflight, assetStem, exportFilename, audioState: () => ({ request: audioRequest, hasSource: Boolean(audio.source), hasDemo: state.demoOn, demoStep: audio.demoStep, hasDemoCompressor: Boolean(audio.demoCompressor), hasMic: Boolean(audio.micStream), hasTab: Boolean(audio.tabStream), sourceOutcome: audioSourceOutcome, sourceHistory: structuredClone(audioSourceEvents), outputGain: audio.gain?.gain.value, bands: structuredClone(state.audioBands), bandsReady: state.audioBandsReady, peak: audioPeakTelemetry(), peakSession: audioPeakSessionTelemetry(), beatPulse: state.beatPulse }) });
...
window.__phosphorTest.restoreRehearsalReportCache = restoreRehearsalReportCache;
boot();
