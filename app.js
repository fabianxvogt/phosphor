...
function rendererReadoutText() {
  ...
  if (scene().kind === 'geometry' && (state.demoOn || state.audioBandsReady)) {
    ...
    const beatWidth = Math.max(1, Math.round(profile.cathedralWidth * .95));
    const beatHeight = Math.max(1, Math.round(profile.cathedralHeight * .95));
