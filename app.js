...
function cathedralRenderPlan(profile = outputProfile(), beatLoad = state.demoOn || state.audioBandsReady) {
  ...
  const width = Math.min(profile.width, Math.max(1, Math.round(profile.cathedralWidth * profileScale * beatScale)));
  const height = Math.min(profile.height, Math.max(1, Math.round(profile.cathedralHeight * profileScale * beatScale)));
