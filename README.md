...
- `Full · 960×600 / 60 target` and `Low · 480×300 / 30 target` resize the actual canvas and reduce expensive sampling/particle/organism budgets immediately. They are targets, not measured FPS claims. Frame manifests carry the active output profile and optional per-scene median/p95 frame-time samples when a real runtime has produced them. Still capture, WebM recording with active local/demo/microphone audio mixed into the stream, and a versioned offline frame manifest are available when supported; without an audio source, recording remains a visual-only fallback.
