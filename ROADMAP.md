...
- Focus-mode Full-profile Julia now requests a bounded 1440×900 WebGL backing cap, or a 640×400 CPU detail pass when WebGL is unavailable, reducing the common enlargement to a bounded 1.5× target while preserving the normal Full output profile and explicit renderer diagnostics; canonical HD remains 1920×1200.
...
- Rehearsal source dock now shows a bounded 20-minute active-audio progress meter tied to the pass snapshot and resets on source stop/switch, keeping interrupted runs from appearing continuous.
