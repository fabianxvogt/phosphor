...
test('new family renderers keep fixed bounded resources in source', () => {
  ...
  assert.match(advanced, /ctx\.imageSmoothingQuality = 'high'; ctx\.drawImage\(surface/, 'Julia WebGL compositing requests high-quality resampling');
  assert.match(advanced, /ctx\.imageSmoothingQuality = 'high'; ctx\.drawImage\(buffer/, 'Julia CPU compositing requests high-quality resampling');
  ...
  assert.match(styles, /\.focus-mode \.stage-wrap\.julia-quality-fit,\.focus-mode \.stage-wrap\.fractal-quality-fit \{[^}]*height:auto;[^}]*min-height:0;[^}]*aspect-ratio:16 \/ 10;/, 'Focus quality stages preserve the 16:10 render aspect ratio while capped');
  ...
  assert.match(app, /function cathedralRenderPlan\(profile = outputProfile\(\), beatLoad = state\.demoOn \|\| state\.audioBandsReady\)/, 'Cathedrals centralizes its quality-aware raster plan');
  assert.match(cathedralRenderer, /const plan = cathedralRenderPlan\(profile, beatLoad\)/, 'Cathedrals uses the quality-aware raster plan');
  ...
  assert.match(core, /const strength = clamp\(amount, 0, \.4\)/, 'Cathedrals keeps the edge helper amount linear and bounded');
  assert.match(cathedralRenderer, /cathedralEdgeEnhance\(data, width, height, cathedralLumaCache, \(\.035 \+ surface \* \.025\) \* 2\.5\)/, 'Cathedrals lifts its low-strength edge signal at the call site');
  ...
  assert.match(app, /const safeWeight = clamp\(normalizedBeatValue\(weight\), \.6, 1\.5\)/, 'demo kick weight is bounded before scheduling');
  ...
  assert.match(app, /function audioMappingForScene\(id\)/, 'runtime modulation shares the validated mapping path');
  assert.match(app, /const mapping = audioMappingForScene\(id\)/, 'scene rendering fails closed on invalid beat mappings');
  assert.match(app, /const validBand = \['low', 'mid', 'high'\]\.includes\(band\)/, 'beat coverage validates mapped audio bands');
  assert.match(app, /const validAmount = Number\.isFinite\(amount\) && amount > 0 && amount <= 1/, 'beat coverage validates bounded modulation amounts');
  ...
  assert.match(app, /function startAudioPlayback\(/, 'primary sound control starts a source when none is active');
  ...
  assert.match(app, /function setOfflineControlsLockedWithKickWeight\(job, locked\)/, 'offline rendering locks the demo kick weight control');
  ...
  assert.match(html, /id="demoAudioButton"[^>]*aria-pressed="false"[^>]*aria-describedby="audioStatus beatReadout beatBarReadout beatNextReadout audioCoverageReadout audioHeadroomReadout(?: audioSessionReadout)?(?: audioBeatTelemetryReadout)? audioSourceHistoryReadout(?: demoKickWeightOutput)?"/, 'demo source announces its state, beat bar, current/next beat, coverage, headroom, run, onset telemetry, source history, and kick weight');
  assert.match(html, /id="audioFileInput"[^>]*aria-describedby="audioStatus beatReadout beatBarReadout beatNextReadout audioCoverageReadout audioHeadroomReadout(?: audioSessionReadout)?(?: audioBeatTelemetryReadout)? audioSourceHistoryReadout(?: demoKickWeightOutput)?"/, 'file source references its status, beat bar, current/next beat, coverage, headroom, run, onset telemetry, source history, and kick weight');
