...
test('session repair validates transactionally, migrates legacy saves, and preserves cue/lineage snapshots', async () => {
  ...
  assert.equal(api.audioState().bandsReady, true);
  api.stopAudioSource(); assert.equal(document.getElementById('audioStatus').textContent, 'No music connected'); assert.equal(api.rehearsalReport().audio.status, 'idle', 'manual source stop returns the audio outcome to idle'); assert.equal(document.getElementById('audioRunProgress').value, 0, 'stopping a source resets the sustained run progress'); assert.equal(document.getElementById('audioRunProgress')['aria-valuetext'], '0:00 of 20:00', 'stopping a source resets the sustained run label'); api.updateAudioLevel(); assert.equal(api.audioState().bandsReady, false, 'stopped sources cannot repopulate analysed bands'); assert.equal(api.audioResponseLevel('feedback'), 0, 'stopped sources clear family response meters'); api.stopAudioSource('Shared tab ended · choose another source'); assert.equal(document.getElementById('audioStatus').textContent, 'Shared tab ended · choose another source'); assert.equal(api.rehearsalReport().audio.status, 'ended', 'ended source status remains explicit after cleanup');
