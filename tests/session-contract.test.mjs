...
test('session repair validates transactionally, migrates legacy saves, and preserves cue/lineage snapshots', async () => {
  ...
  assert.equal((document.getElementById('beatPattern').innerHTML.match(/data-beat-step=/g) || []).length, 16, 'demo pattern monitor renders every 16th-note step');
  assert.equal(document.getElementById('beatPattern')['aria-label'], 'Demo 16-step pattern preview; inactive for the current source', 'demo pattern monitor stays explicitly inactive without the demo source');
  ...
  assert.equal(document.getElementById('beatPattern')['aria-label'], 'Demo 16-step pattern; step 01 is active', 'demo pattern monitor follows the scheduled step');
  ...
  assert.equal(document.getElementById('beatPattern')['aria-label'], 'Demo 16-step pattern; step 04 is active', 'demo pattern monitor follows a ghost percussion step');
  api.setTestBeatPulse(0, 0);
