...
class FakeElement {
  ...
  querySelectorAll(selector) { if (selector === '[data-scene]') return this.children.filter((child) => child.dataset.scene !== undefined); if (selector === '[data-preset]') return this.children.filter((child) => child.dataset.preset !== undefined); if (selector === '[data-label-cue]') return this.children.filter((child) => child.dataset.labelCue !== undefined); if (selector === '[data-duration-cue]') return this.children.filter((child) => child.dataset.durationCue !== undefined); if (selector === '[data-preview-cue]') return this.children.filter((child) => child.dataset.previewCue !== undefined); if (selector === '[data-duplicate-cue]') return this.children.filter((child) => child.dataset.duplicateCue !== undefined); if (selector === '[data-move-cue]') return this.children.filter((child) => child.dataset.moveCue !== undefined); if (selector === '[data-remove-cue]') return this.children.filter((child) => child.dataset.removeCue !== undefined); if (selector === '[data-beat-step]') return this.children.filter((child) => child.dataset.beatStep !== undefined); return []; }
...
test('session repair validates transactionally, migrates legacy saves, and preserves cue/lineage snapshots', async () => {
  ...
  assert.equal(document.getElementById('focusAudioButton').hidden, true, 'Focus audio action stays hidden outside Focus mode');
  ...
  assert.equal(document.getElementById('stageBeatReadout').textContent, 'BEAT IDLE', 'Focus stage beat readout starts idle');
  assert.equal(document.getElementById('stageBeatReadout').dataset.active, 'false', 'Focus stage beat readout starts inactive');
  ...
  assert.equal(beatPatternCells().length, 16, 'demo pattern monitor renders every 16th-note step');
  assert.equal(beatPatternCells().filter((cell) => cell.dataset.active === 'true').length, 0, 'demo pattern monitor starts with no active step');
  assert.equal(beatPattern['aria-label'], 'Demo 16-step pattern preview; inactive for the current source', 'demo pattern monitor stays explicitly inactive without the demo source');
  ...
  assert.equal(document.getElementById('setTimingButton').disabled, false, 'set timing probe is available when the stage is idle');
  ...
  assert.equal(document.getElementById('rehearsalReportImportSourceHistoryReadout').textContent, 'No loaded source history', 'loaded report source history starts idle');
  ...
  assert.equal(document.getElementById('audioSourceHistoryReadout').textContent, 'SOURCE HISTORY · 0 PATHS · SWITCHING NOT OBSERVED', 'audio-source history starts explicit about switching evidence');
  assert.equal(document.getElementById('rehearsalSourceHistoryReadout').textContent, 'SOURCE HISTORY · 0 PATHS · SWITCHING NOT OBSERVED', 'rehearsal card mirrors the initial source history');
  ...
  assert.equal(document.getElementById('focusAudioButton').hidden, false, 'Focus exposes the demo start action when no audio source is active');
  assert.equal(document.getElementById('focusAudioButton')['aria-label'], 'Start the dark techno demo beat', 'Focus audio action names the local demo');
  ...
  assert.equal(document.getElementById('qualityBadge').classList.contains('quality-warning'), true, 'stage quality badge highlights the sharper Julia correction');
  assert.equal(document.getElementById('qualityBadge').title, 'HD output is available for sharper Julia detail', 'stage quality badge explains the sharper Julia correction on hover');
  ...
  assert.equal(document.getElementById('focusAudioButton').hidden, true, 'leaving Focus hides the audio action');
  ...
  assert.equal(document.getElementById('qualityBadge').classList.contains('quality-warning'), false, 'stage quality badge clears the correction highlight after switching profiles');
  assert.equal(document.getElementById('qualityBadge').title, '', 'stage quality badge clears its correction hint after switching profiles');
  ...
  assert.equal(setTiming.sampledScenes, api.sceneDefs.length, 'set timing probe measures every visual family');
  assert.equal(setTiming.sampleCount, api.sceneDefs.length * 3, 'set timing probe records three samples per family');
  ...
  assert.equal(reportCache.qualityAB, null, 'report cache stays explicit when no quality A/B was captured');
  ...
  assert.match(document.getElementById('qualityABReadout').textContent, /(HD (HIGHER DETAIL|REDUCES UPSCALE)|FULL HIGHER DETAIL|SAME BACKING DETAIL)/, 'reopened cache preserves actionable quality guidance');
  assert.match(document.getElementById('qualityABReadout')['aria-label'], /Saved Julia quality A\/B .*recommendation (hd higher detail|hd reduces upscale|full higher detail|same backing detail)/, 'reopened cache keeps quality guidance accessible');
  assert.doesNotMatch(document.getElementById('qualityABReadout').textContent, /STALE$/, 'matching quality cache reopen stays fresh');
  api.importRehearsalReport(rehearsalReport);
  assert.equal(document.getElementById('qualityABReadout').textContent, 'Quality A/B available on Julia', 'explicitly loaded reports without A/B evidence suppress older cache guidance');
  api.clearRehearsalReportImport();
  const malformedQualityReportCache = structuredClone(reportCache); malformedQualityReportCache.qualityAB = { format: 'phosphor-quality-ab-v1', version: 1 };
  assert.equal(api.restoreRehearsalReportCache(malformedQualityReportCache), false, 'report cache rejects malformed quality A/B evidence');
  ...
  assert.equal(document.getElementById('rehearsalReportImportSourceHistoryReadout').textContent, 'LOADED SOURCE HISTORY · 0 PATHS · NO AUDIO · LAST NO AUDIO IDLE', 'loaded report exposes the captured empty source history');
  ...
  assert.equal(document.getElementById('rehearsalReportImportSourceHistoryReadout').textContent, 'LOADED SOURCE HISTORY · 2 PATHS · MIC · DEMO · MULTIPLE PATHS SEEN · LAST DEMO ACTIVE', 'loaded report exposes the captured source history and last outcome');
  ...
  assert.equal(document.getElementById('rehearsalReportImportSourceHistoryReadout').textContent, 'No loaded source history', 'clearing a loaded report resets its source history readout');
  ...
  assert.deepEqual(api.phaseRenderPlan(), { width: 256, height: 160, scale: 4, sourceWidth: 64, sourceHeight: 40 }, 'Full Phase output quadruples the field raster without changing simulation state'); quality.value = '720'; quality.dispatchEvent({ type: 'change' }); assert.deepEqual(api.phaseRenderPlan(), { width: 128, height: 80, scale: 2, sourceWidth: 64, sourceHeight: 40 }, 'Low Phase output doubles the field raster'); quality.value = 'native'; quality.dispatchEvent({ type: 'change' }); assert.deepEqual(api.phaseRenderPlan(), { width: 384, height: 240, scale: 6, sourceWidth: 64, sourceHeight: 40 }, 'HD Phase output uses a bounded supersampled raster'); quality.value = '1080'; quality.dispatchEvent({ type: 'change' });
  const cathedralFocusButton = document.getElementById('focusButton'); const cathedralFocusWasOn = cathedralFocusButton['aria-pressed'] === 'true'; if (!cathedralFocusWasOn) cathedralFocusButton.click(); assert.deepEqual(api.cathedralRenderPlan(undefined, false), { width: 240, height: 150, scale: 1.5, sourceWidth: 160, sourceHeight: 100, beatLoad: false }, 'Full Cathedrals Focus uses a sharper bounded raster'); assert.deepEqual(api.cathedralMaxSteps(undefined, false), 36, 'Full Cathedrals Focus lowers idle march steps to bound the sharper raster'); assert.deepEqual(api.cathedralRenderPlan(undefined, true), { width: 160, height: 100, scale: 1, sourceWidth: 160, sourceHeight: 100, beatLoad: true }, 'Full Cathedrals beat raster keeps its native bounded raster'); cathedralFocusButton.click(); assert.deepEqual(api.cathedralRenderPlan(undefined, false), { width: 160, height: 100, scale: 1, sourceWidth: 160, sourceHeight: 100, beatLoad: false }, 'Full Cathedrals normal mode keeps its native bounded raster'); assert.deepEqual(api.cathedralMaxSteps(undefined, false), 48, 'Full Cathedrals idle march keeps the quality step budget'); quality.value = 'native'; quality.dispatchEvent({ type: 'change' }); assert.deepEqual(api.cathedralRenderPlan(undefined, false), { width: 240, height: 150, scale: 1, sourceWidth: 240, sourceHeight: 150, beatLoad: false }, 'HD Cathedrals output stays native-sized'); assert.deepEqual(api.cathedralMaxSteps(undefined, false), 48, 'HD Cathedrals idle march keeps its quality step budget'); quality.value = '1080'; quality.dispatchEvent({ type: 'change' }); if (cathedralFocusWasOn) cathedralFocusButton.click();
  ...
  if (document.getElementById('focusAudioButton').hidden) document.getElementById('focusButton').click();
  assert.equal(document.getElementById('focusAudioButton').hidden, false, 'offline lock fixture starts with the Focus audio action visible');
  window.__phosphorMetrics.sceneTimes[0].push(8, 10, 12);
  ...
  assert.equal(api.audioState().bandsReady, true);
  api.stopAudioSource(); assert.equal(document.getElementById('audioStatus').textContent, 'No music connected'); assert.equal(api.rehearsalReport().audio.status, 'idle', 'manual source stop returns the audio outcome to idle'); assert.match(document.getElementById('rehearsalPassReadout').textContent, /^PASS IN PROGRESS · NO AUDIO · no audio run/, 'stopping a source immediately refreshes the rehearsal pass source'); assert.equal(document.getElementById('audioRunProgress').value, 0, 'stopping a source resets the sustained run progress'); assert.equal(document.getElementById('audioRunProgress')['aria-valuetext'], '0:00 of 20:00', 'stopping a source resets the sustained run label'); assert.equal(document.getElementById('audioRunProgressReadout').textContent, '20:00 LEFT', 'stopping a source resets the remaining-time label'); api.updateAudioLevel(); assert.equal(api.audioState().bandsReady, false, 'stopped sources cannot repopulate analysed bands'); assert.equal(api.audioResponseLevel('feedback'), 0, 'stopped sources clear family response meters'); api.stopAudioSource('Shared tab ended · choose another source'); assert.equal(document.getElementById('audioStatus').textContent, 'Shared tab ended · choose another source'); assert.equal(api.rehearsalReport().audio.status, 'ended', 'ended source status remains explicit after cleanup');
  ...
  assert.equal(document.getElementById('audioSourceHistoryReadout').textContent, 'SOURCE HISTORY · 1 PATH · MIC', 'source dock exposes the first connected path');
  assert.equal(document.getElementById('rehearsalSourceHistoryReadout').textContent, 'SOURCE HISTORY · 1 PATH · MIC', 'rehearsal card mirrors the first connected path');
  ...
  assert.equal(document.getElementById('stageBeatReadout').textContent, 'BEAT 01/16 · KICK+SUB · 100%', 'Focus stage mirrors the first active beat step');
  assert.equal(document.getElementById('stageBeatReadout')['aria-label'], 'Current demo beat step 01/16: kick and sub; response 100%', 'Focus stage beat readout exposes the active step accessibly');
  assert.equal(document.getElementById('stageBeatReadout').dataset.active, 'true', 'Focus stage beat readout marks an active Demo step');
  ...
  assert.equal(document.getElementById('stageBeatReadout').textContent, 'BEAT 04/16 · HAT+SUB+PERC · 30%', 'Focus stage follows later beat voices');
