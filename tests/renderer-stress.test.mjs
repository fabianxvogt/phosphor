...
function runBoundedRendererStress(seed, frames = 900) {
  ...
  const organisms = new Float32Array(64 * 5);
  ...
  for (let i = 0; i < organisms.length; i += 5) { organisms[i] = rand(); organisms[i + 1] = rand(); organisms[i + 2] = (rand() - .5) * .01; organisms[i + 3] = (rand() - .5) * .01; organisms[i + 4] = .01 + rand() * .02; }
  phaseField.fill(.5);
  ...
  for (let frame = 0; frame < frames; frame += 1) {
    ...
    for (let i = 0; i < 64; i += 1) { const index = i * 5; organisms[index] = (organisms[index] + organisms[index + 2] + 1) % 1; organisms[index + 1] = (organisms[index + 1] + organisms[index + 3] + 1) % 1; organisms[index + 2] = clamp(organisms[index + 2] + Math.sin(frame * .02 + i) * .0001, -.02, .02); organisms[index + 3] = clamp(organisms[index + 3] + Math.cos(frame * .017 + i) * .0001, -.02, .02); }
